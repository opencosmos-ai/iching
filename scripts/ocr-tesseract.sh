...
# (scripts/ocr-pdf.swift), which needs no install. Measured over the whole
...
# one word in five is wrong. See scripts/ocr-consensus.ts.
#
...
#
# RUN
#   scripts/ocr-tesseract.sh <in.pdf> <out.txt> [from] [to]
#
set -euo pipefail
...
swiftc -O "$ROOT/scripts/page-ink.swift" -o "$INK" -framework PDFKit -framework AppKit
