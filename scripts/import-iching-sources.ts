 ...
 * file says which one it is. See sources/PROVENANCE.md.
 *
...
async function importZhouyi() {
  ...
  console.log(`  → sources/zhouyi/01.md … 64.md`)
}
...
async function importLegge() {
  ...
  console.log(`  → sources/legge-1882/01.md … 64.md  (${written} files)`)
}
...
async function importHarlez() {
  ...
  console.log(`  → sources/harlez-1889/01.md … 64.md  (${written} files)`)
}
...
// So the text has to arrive by hand — see scripts/harvest-hathitrust.js.
//
    ...
    if (err.code === 'ENOENT') {
      throw new Error(
        'pdftotext is not installed, and it is the only thing here that reads a PDF.\n' +
          ...
          '    Or OCR the scan — see scripts/ocr-pdf.swift — and pass the .txt.',
      )
    }
    throw e
  }
 ...
 *   scripts/ocr-tesseract.sh  (primary)
 *   scripts/ocr-pdf.swift     (witness; --scale 3 is right, see its header)
 */
...
async function importMcClatchie() {
  ...
  if (!existsSync(source)) {
    ...
    console.log('    Produce it with scripts/ocr-tesseract.sh, or pass --from <file>.')
    return
  }
  ...
  console.log(`  → sources/mcclatchie-1876/01.md … 64.md`)
  ...
  writeFileSync(
    ...
    [
      ...
      '# columnar type, which the OCR in scripts/ocr-pdf.swift cannot read,',
      ...
      '#',
      '# Open one with:',
      '#   swiftc -O scripts/page-ink.swift -o /tmp/page-ink -framework PDFKit -framework AppKit',
      '#   /tmp/page-ink sources/.cache/mcclatchie-1876.pdf --export <seq> --dir /tmp',
      '#',
...
#   swiftc -O scripts/page-ink.swift -o /tmp/page-ink -framework PDFKit -framework AppKit
#   /tmp/page-ink sources/.cache/mcclatchie-1876.pdf \\
#       --export 14,15,16,17 --dir sources/mcclatchie-1876/figures --scale 4
#
 ...
 *
 * Produced by `scripts/ocr-vision-adjudicate.ts`, which shows a model the
...
function mcDisputedYaml(byHexagram: Map<number, Dispute[]>, total: Dispute[], corroborated: number, tokens: number,
  ...
  const head = [
    ...
    '#',
    '# Open a page with:  scripts/page-ink.swift <pdf> --export <seq> --dir . --scale 8',
    '#',
