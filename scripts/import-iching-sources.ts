...
/**
 * Write a vendored file — unless the only difference from what is already there
 * is the run date. `transcribed:` and the `Generated …` stamps say when the text
 * was last taken, and a re-run that changes no text has not taken it again; left
 * to TODAY, every offline re-run touched 268 files and hid any real change among
 * them. The stamp moves when the text does.
 */
const STAMP = /^(transcribed:|\s*generated:|# Generated|\*Generated by).*$/gm
const unstamped = (s: string) => s.replace(STAMP, l => l.replace(/\d{4}-\d{2}-\d{2}/g, 'DATE'))
function writeVendored(path: string, text: string) {
  if (existsSync(path) && unstamped(readFileSync(path, 'utf8')) === unstamped(text)) return
...
async function importZhouyi() {
  ...
  for (const hex of HEX) {
    ...
    writeVendored(join(out, `${String(hex.number).padStart(2, '0')}.md`), zhouyiFile(hex, z, title, revid))
  }
...
async function importWings() {
  ...
  for (const wing of WINGS) {
    ...
    const md = body
      ...
      .join('\n\n')
      .replace(/\n{3,}/g, '\n\n')
    writeVendored(join(out, `${wing.slug}.md`), fm + md.trim() + '\n')
...
async function importLegge() {
  ...
  for (let n = 1; n <= 64; n++) {
    ...
    writeVendored(join(out, `${String(n).padStart(2, '0')}.md`), leggeFile(n, legge, p ?? null, pct))
    written++
  }
...
async function importLocks() {
  ...
  const md = [
    ...
    '',
    `*Generated by \`npm run import-sources\` on ${TODAY}. \`terms.yaml\` beside this file is vendored verbatim from`,
  ...
  writeVendored(join(out, 'README.md'), md)
...
async function importHarlez() {
  ...
  for (let n = 1; n <= 64; n++) {
    ...
    writeVendored(join(out, `${String(n).padStart(2, '0')}.md`), harlezFile(n, h))
    written++
  }
...
async function importMcClatchie() {
  ...
  if (witness.length) {
    ...
    else console.log('  · no scan pass yet — run npx tsx scripts/ocr-vision-adjudicate.ts to settle the rest')
  ...
  for (const sec of SECTIONS) {
    ...
    writeVendored(join(out, `${sec.slug}.md`), mcclatchieSection(sec, paras, own))
  ...
  writeVendored(join(out, 'appendix.md'), mcclatchieAppendix(appParas, appendix))
  ...
  if (witness.length) {
    ...
    writeVendored(join(out, 'disputed.yaml'),
  ...
  writeVendored(join(out, 'plates.yaml'), MC_PLATES)
  writeVendored(
